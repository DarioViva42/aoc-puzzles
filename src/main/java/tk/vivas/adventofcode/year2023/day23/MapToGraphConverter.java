...
import static tk.vivas.adventofcode.year2023.day23.TrailTile.SLOPE_WEST;
...
class MapToGraphConverter {
    ...
    MapToGraphConverter(TrailTile[][] map) {
        this.map = map;

        sizeX = map[0].length;
        sizeY = map.length;
        start = Arrays.stream(map).map(e -> e[0]).toList().indexOf(SLOPE_SOUTH);
        end = Arrays.stream(map).map(e -> e[sizeY - 1]).toList().indexOf(SLOPE_SOUTH);
    }

    PathSection toGraph() {
        return walkPaths(start, 0);
    }
